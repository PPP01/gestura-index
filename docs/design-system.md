...
- **Favicon:** die **App-Kachel** `icon{16,32,48}-tile.png` – wörtliche Kopien aus `icons/` des Extension-Repos, wo `-tile` und `-darktile` seit dem Icon-Wechsel dieselbe Datei sind (Verlaufs-Kachel `#5b9cf6` → `#7c5ce6` → `#a34fd0` mit weißer Hand, Hand auf 1,24× vergrößert, damit sie bei 16 und 32 px trägt). Sie bringt ihren eigenen Grund mit und ist damit in heller **und** dunkler Tab-Leiste lesbar – **keine** Weiche über `media="(prefers-color-scheme: …)"`. Die Weiche funktioniert (Chromium wertet sie aus und lädt nur das passende Bild), wäre aber nur für ein freistehendes Icon nötig und müsste dann am **OS**-Theme hängen, nicht am Theme der Seite: das Icon sitzt in der Tab-Leiste, deren Farbe die Seite nicht bestimmt. `static/favicon.png` ist die 32er-Fassung als Pfad-Rückfall.
