...
- **Logo im Header:** die **reine Hand**, ohne Kachel und ohne Rahmen – im hellen Thema `icon128.png` (getönte Verlaufs-Hand, entspricht `varianten/hand-verlauf-dunkel-072` des Logo-Pakets), im dunklen `icon128-dark.png` (weiße Hand). Beide sind eng beschnitten (Zeichnung 125×128 von 128×128) und füllen ihr 36×36-Kästchen fast ganz; gerendert 35,16×36 px. Auf der Vergleichsseite dasselbe bei 20×20. `.logo-img` ist vollständig in den Komponenten definiert, nicht global, und trägt weder Hintergrund noch Rundung noch Schatten – ohne Hintergrund würde beides nichts tun. Daneben der Schriftzug **Gestura** plus Badge (Muster `.version`-Badge; auf der Website z. B. »Index«).

  **Bewusste Abweichung von der Extension:** Deren Options-Kopf setzt dieselbe Hand auf eine CSS-Kachel (`css/common.css`, `.logo-img`: 5px Padding, 10px Rundung, Verlauf `#ffffff→#f1f9ff` bzw. `#2a3040→#1c222e`, Schatten). Die Website zeigt die Hand frei und dadurch größer. Wer die Kachel zurückholt, holt das Padding mit und verkleinert die Hand auf 26px.
- **Favicon:** das **freistehende Hand-Paar** `icon48-light.png` (fast schwarze Hand, für helle Tab-Leisten) und `icon48-dark.png` (weiße Hand, für dunkle) – wörtliche Kopien aus `icons/` des Extension-Repos, dort in `static/` gespiegelt. Beide sind EINFARBIG mit der Schattierung im Alphakanal: bei 16px verliert die Verlaufs-Hand gegen eine weiße Tab-Leiste die Fingerspitzen. Umgeschaltet wird per Inline-Script in `app.html` nach `prefers-color-scheme` – also nach dem **Browser**-Theme, nicht nach dem der Seite: das Icon sitzt in der Tab-Leiste, deren Farbe die Seite nicht bestimmt. Link und Script stehen vor `%sveltekit.head%`, damit schon der erste Paint das richtige Icon trägt; das Script hört zusätzlich auf Themewechsel im laufenden Tab. Muster und Begründung aus `js/page-favicon-theme.js` der Extension: `<link media="(prefers-color-scheme)">` wird zwar von Chromium ausgewertet (gemessen), deckt aber den laufenden Wechsel nicht ab. **Nicht** die Verlaufs-Kachel aus dem Logo-Paket – die ist das App-Icon für Store und Toolbar, nicht das Favicon.
