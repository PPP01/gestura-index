...
| `elements.css` | `.container` `.card` `.icon-tile` `.badge` `.filter-bar` `.toggle` | ja |
| `pages.css` | `.rail-*`, `.page-hero`, `.page-lead`, `.text-page` | ja |
...
- **Karten/Sektionen:** `border-radius: 20px`, Hintergrund `--bg-secondary`, `1px solid var(--section-border)`, Schatten `--section-shadow 0 1px 3px` – als `.card` in `elements.css`. Zeilen darin nach dem `.setting-row`-Muster (14px vertikales Padding, `border-top: 1px solid var(--border-color)`).
