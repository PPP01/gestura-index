...
- **Logo im Header:** die **CSS-Kachel** mit freistehender Hand darauf – genau wie im Options-Kopf der Extension (`css/common.css`, `.logo-img`). 36×36, `border-radius: 10px`, `padding: 5px`, Verlauf `linear-gradient(145deg, #ffffff, #f1f9ff)` im hellen und `#2a3040 → #1c222e` im dunklen Thema, dazu `box-shadow: 0 1px 3px` (0.12 hell / 0.35 dunkel). Darauf `icon128.png` (getönte Verlaufs-Hand) bzw. `icon128-dark.png` (weiße Hand), beide freistehend auf Transparenz. Auf der Vergleichsseite dasselbe maßstäblich bei 20px (Padding 3px, Rundung 6px). Die Außenabstände der Quelle (`margin-top: 4px`, `margin-inline-end: 12px`) bleiben weg – die Markenzeile setzt ihren Abstand über `gap`. `.logo-img` ist vollständig in den Komponenten definiert, nicht global. Daneben der Schriftzug **Gestura** plus Badge (Muster `.version`-Badge; auf der Website z. B. »Index«).
