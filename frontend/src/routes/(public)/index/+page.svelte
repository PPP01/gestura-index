<script lang="ts">
	...
	// Sprach-Weiche: die nackte Katalog-URL /index auf die lokalisierte URL lenken.
	if (browser && location.pathname === '/index') {
		location.replace(localizeHref('/index', { locale: getLocale() }));
	}
	...
	function updateUrl(next: OnepagerFilter) {
		...
		goto(localizeHref(`/index${qs ? `?${qs}` : ''}`), { replaceState: true, keepFocus: true, noScroll: true });
	}
